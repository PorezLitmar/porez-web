...
const BoardSearchCriteriaForm = ({searchHandler}: {
    ...
    const createCriteria = () => {
        return {
            ...
            boardCode: boardCode.length > 0 ? boardCode : undefined,
            structureCode: structureCode.length > 0 ? structureCode : undefined,
            lengthFrom: parseNumber(lengthFrom),
            lengthTo: parseNumber(lengthTo),
        ...
        <div className="flex flex-col w-full gap-2">
            <div className="join join-horizontal w-full">
                <input
                    ...
                    placeholder="Kód, názov, kód dosky alebo kód štruktúry"
                    value={searchField}
            ...
            {extended &&
                <>
                    <div className="grid grid-cols-4 gap-2 w-full">
                        ...
                        <FormInput
                            name="boardCode"
                            placeholder="Kód dosky"
                            value={boardCode}
                            onChange={event => setBoardCode(event.target.value)}
                        />
                        <FormInput
                            name="structureCode"
                            placeholder="Kód štruktúry"
                            value={structureCode}
                            onChange={event => setStructureCode(event.target.value)}
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-8 gap-2 w-full">
                        ...
                        <FormInput
                            ...
                            value={lengthFrom}
                            onChange={event => setLengthFrom(event.target.value)}
                        />
                        <FormInput
                            name="lengthTo"
                            placeholder="Dĺžka do"
                            value={lengthTo}
                            onChange={event => setLengthTo(event.target.value)}
                        />
                        <FormInput
                            name="widthFrom"
                    ...
                    {categoryState?.boardCategories &&
                        <div className="flex flex-col w-full">
                            {categoryState?.boardCategories.map(category =>
                                <SelectCategoryItem
                                    key={category.id}
