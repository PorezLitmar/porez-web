...
import {
    type FormOrderBoard,
    toFormOrderBoardProduct,
    toFormOrderBoardValues,
    validateFormOrderBoardProduct,
    validateFormOrderBoardValues
} from './form-order-board';
import FormBoardEditor from './board/form-board-editor';
import FormInput from '../form/form-input';
import {AppContext} from '../../state';
import OrderCommonForm from './order-commom-form';

export type OrderBoardFormHandle = {
    validate: () => Promise<boolean>;
    getProduct: () => Promise<Product>;
}

type OrderBoardFormProps = {
...
const OrderBoardForm = forwardRef<OrderBoardFormHandle, OrderBoardFormProps>((props, ref) => {
    const appState = useContext(AppContext);

    const [values, setValues] = useState<FormOrderBoard>({
        boardId: '',
        length: '',
        width: '',
    ...
    const [errors, setErrors] = useState<FieldErrors<FormOrderBoard>>({});
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const values = await toFormOrderBoardValues(props.product);
            props.setProduct(await toFormOrderBoardProduct(values, props.strictDecorOrientation));
            setValues(values);
        })().finally(() => {
    ...
    const validate = async () => {
        ...
        const ve = await validateFormOrderBoardValues(actualValues, appState);

        const product = await toFormOrderBoardProduct(actualValues, props.strictDecorOrientation);
        const pe = await validateFormOrderBoardProduct(product, props.getBoard, props.getEdge);
    ...
    const getProduct = async (): Promise<Product> => {
        return toFormOrderBoardProduct({...values}, props.strictDecorOrientation);
    }
...
OrderBoardForm.displayName = 'OrderBoardForm';

export default OrderBoardForm;
