/* tslint:disable */
/* eslint-disable */
import {ErrorCode} from '../model/porez';
import type {ErrorMessage} from '../model/porez';
...
export const postFormData = async <T>(path: string, formData: FormData, token?: string): Promise<ClientResponse<T>> => {
    const authorization = createAuthorization(token);
...
export const putFormData = async <T>(path: string, formData: FormData, token?: string): Promise<ClientResponse<T>> => {
    const authorization = createAuthorization(token);
...
export const patchFormData = async <T>(path: string, formData: FormData, token?: string): Promise<ClientResponse<T>> => {
    const authorization = createAuthorization(token);
